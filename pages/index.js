        ...
        <div className="relative">
            ...
            <div className={` bg-blend-overlay bg-cover bg-opacity-60 bg-black min-h-screen`}>
                <Head>
                    ...
                    <meta name="description" content="The WOPTY is a digital platform to inspire and be inspired by positive messages. Join the movement. Spread positivity for yourself and others. Contribution and usage are completely free and anonymous" />
                                        ...
                                        <div key={index}>
                                            {post.length &&
                                                post.map((onePost, index) => {
                                                    return (
                                                        <p  key={onePost._id} className={`opacity-60 my-8 mx-5 ml-8 bg-white ${color[Math.round(Math.random() * 4)]} post text-overflow max-w-20 transform min-h-8 bg-white p-3 rounded-md ${fonts[1]} ${fontWeight[8]}`} onClick={()=>showPopup(onePost.post)} >
                                                            {onePost.post}
                                                        </p>
                                                    );
                                                })}
                        ...
                        posts.map((post, index) => {
                            return (
                                <div style={{ gridArea: position[index], background:"white" }} className={`opacity-60 flex items-center justify-center post transform rounded-md transition duration-500 ease-in-out hover:scale-110 shadow-lg hover:shadow-none`} key={post._id}>
                                    <p className={`opacity-100 bg-white ${color[Math.round(Math.random() * 4)]}  max-w-20  min-h-8 bg-white p-3 rounded-md text-center ${fonts[1]} ${fontWeight[8]}  message`}>{post.post}</p>
                                </div>
